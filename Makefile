...
start_build:
	@echo "▶ Run Build $(NAME)"
	@echo "$(shell date +%H:%M:%S) ${YELLOW}🟡 Starting minishell compilation...${RESET}"

end_build:
	@echo "$(shell date +%H:%M:%S) ${GREEN}🟢 Build finished successfully.${RESET}"
...
clean:
	@clear
	@echo "▶ Run Clean"
	@echo "$(shell date +%H:%M:%S) ${BLUE}🟡 Cleaning object files...${RESET}"
	@$(MAKE) -s -C libft clean > /dev/null
	@rm -f $(OBJS)
	@echo "$(shell date +%H:%M:%S) ${GREEN}🟢 Clean done.${RESET}"
	@echo "$(RED_BRIGHT)$$HEADER2$(RESET)"

fclean: clean
	@clear
	@echo "▶ Run Fclean"
	@echo "$(shell date +%H:%M:%S) ${BLUE}🟡 Removing binaries and libft...${RESET}"
	@$(MAKE) -s -C libft fclean > /dev/null
	@rm -f $(NAME)
	@echo "$(shell date +%H:%M:%S) ${GREEN}🟢 Fclean done.${RESET}"
	@echo "$(RED_BRIGHT)$$HEADER2$(RESET)"
